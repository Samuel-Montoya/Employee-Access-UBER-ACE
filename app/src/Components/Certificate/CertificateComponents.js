			...
			<PopupBody>
				<BackToSearchContainer>
					...
					<RedeemButton
						color="#63b3ff"
						onClick={() =>
							this.props.updateState(this.props.displayContainer, false)
						}>
						View Certificate
					</RedeemButton>
					<Link style={{ width: '100%' }} to="/search">
						<RedeemButton color="#8E0E34" style={{ cursor: 'pointer' }}>
							Back To Search
						</RedeemButton>
					</Link>
				</BackToSearchContainer>
...
export class RedeemCertificate extends React.Component {
	...
	redeemCertificate = () => {
		...
		axios
			.post('http://localhost:5000/api/updateCertificate', tempCertificate)
			.then(response => {
				...
				<PopupContent>
					...
					{this.state.didRedeem ? (
						<BackToSearch
							...
							displayContainer="showRedeemInfo"
						/>
					) : (
						<PopupBody>
							<RedeemContainer>
								...
								<section>
									...
									<div>
										...
										<input
											...
											onChange={text =>
												this.setState({
													licensePlate: text.target.value.toUpperCase()
												})
											}
									...
									<div>
										...
										<select
											...
											onChange={selected =>
												this.setState({ carState: selected.target.value })
											}>
								...
								<section>
									<div>
										...
										<input
											...
											onChange={text =>
												this.setState({ currentDate: text.target.value })
											}
										/>
									</div>
									...
									<div>
										...
										<input
											...
											onChange={text =>
												this.setState({ storeNumber: text.target.value })
											}
											style={{ alignSelf: 'flex-end' }}
										/>
									</div>
									...
									<div>
										...
										<input
											...
											onChange={text =>
												this.setState({ currentTime: text.target.value })
											}
											style={{ alignSelf: 'flex-end' }}
										/>
									</div>
								...
								this.state.licensePlate ? (
									<RedeemButton
										onClick={this.redeemCertificate}
										color="#42EA1A">
										Redeem
									</RedeemButton>
								) : (
			...
			<PopupContainer>
				<PopupContent>
					...
					{this.state.didUnredeem ? (
						<BackToSearch
							updateState={this.props.updateState}
							displayContainer="showUnRedeemInfo"
						/>
					) : (
						<PopupBody>
							<UnRedeemContainer>
								...
								<textarea
									onChange={text =>
										this.setState({ reasonText: text.target.value })
									}
								/>
								{this.state.reasonText ? (
									<RedeemButton
										color="#48d20e"
										onClick={() => this.unredeemCertificate()}>
										Unredeem
									</RedeemButton>
								) : (
