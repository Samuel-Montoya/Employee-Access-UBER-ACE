...
let CertInformationContainer = styled.section`
  ...
  section {
    ...
    ${props => props.small && `text-align: center;`} h2 {
      font-weight: lighter;
      margin-top: 20px;
      color: rgb(43, 43, 43);
    }
      ...
      axios
        ...
        .then(allCertificates => {
          let certificate = allCertificates.data.filter(
            certificate =>
              certificate.certificate_number === newCertificateNumber
          )[0]
          if (certificate) {
            axios
              .get(
      ...
      <div style={{ height: "100vh" }}>
        ...
        <PageContainer>
          <CertHeaderContainer>
            ...
            <Title>
              CERTIFICATE #{this.state.certificateInfo.certificate_number}
            </Title>
          </CertHeaderContainer>
          ...
          <HeaderButtonContainer>
            ...
            {this.state.certificateInfo.status === "Redeemable" ? (
              <HeaderButton
                color="#48d20e"
                onClick={() => this.setState({ showRedeemInfo: true })}
              >
                Redeem Certificate
              </HeaderButton>
            ) : (
              <HeaderButton
                color="#BC1B4B"
                onClick={() => {
                  this.setState({ showUnRedeemInfo: true })
                }}
              >
                Unredeem Certificate
              </HeaderButton>
            )}
          </HeaderButtonContainer>

          {this.state.showRedeemInfo && (
            <RedeemCertificate
          ...
          <CertInformationContainer>
            ...
            <div>
              ...
              <section style={{ width: "20%", marginRight: "5%" }}>
                <h1>PURCHASE DATE</h1>
                <h2>
                  {moment(this.state.certificateInfo.purchase_date).format("L")}
                </h2>
              </section>
              ...
              <section style={{ width: "25%" }}>
                ...
                <h2>
                  {moment(this.state.certificateInfo.expiration_date).format(
                    "L"
                  )}
                </h2>
              </section>
            ...
            <CertInformationContainer small>
              ...
              <div>
                ...
                <section>
                  <h1>LICENSE PLATE</h1>
                  <h2>
                    {this.state.certificateInfo.license_plate},{" "}
                    {this.state.certificateInfo.vehicle_state}
                  </h2>
                </section>
                ...
                <section>
                  <h1>WHEN</h1>
                  <h2>
                    {moment(this.state.certificateInfo.redemption_date).format(
                      "dddd, MMMM Do YYYY"
                    )}{" "}
                    -{" "}
                    {moment(this.state.certificateInfo.redemption_date).format(
                    ...
                    <br />
                    {moment(
                      this.state.certificateInfo.redemption_date
                    ).fromNow()}
                  </h2>
                </section>
              </div>
